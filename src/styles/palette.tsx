export const palette = {
  primary: '#f70949',
  secondary: '#ebc15c',
  textSecondary: 'rgb(238, 196, 100)',
  textBlack: '#000',
  textRed: 'rgb(230, 93, 93)',
  textWhiteMuted: 'rgba(255, 255, 255, 0.54)',
  textGrey: 'rgb(148, 149, 149)',
  lightGrey: 'rgb(237,238,239)',
  grey: 'rgb(222, 223, 225)',
  gradientPrimaryFirst: 'rgb(247, 9, 73)',
  gradientPrimarySecond: 'rgb(31, 1, 31)',
  gradientSecondaryFirst: 'rgb(255, 214, 99)',
  gradientSecondarySecond: 'rgb(200, 156, 101)',
  background: '#fff',
  backgroundDarker: 'rgb(234, 234, 234)',
  border: 'rgb(204, 204, 204)',
  error: 'rgb(244, 94, 89)',
  shadow: 'rgba(0, 0, 0, 0.12)',
  shadowLight: 'rgba(0, 0, 0, 0.08)',
  transparent: 'transparent',
  white: 'rgb(255, 255, 255)',
  searchBar: 'rgba(255, 255, 255, 0.2)',
  modalTransparent: 'rgba(0,0,0,0.5)',
};

export const gradients = {
  Primary: {
    background: `linear-gradient(180deg, ${palette.gradientPrimaryFirst} 0%, ${palette.gradientPrimarySecond} 100%)`
  },
  Secondary: {
    background: `linear-gradient(180deg, ${palette.gradientSecondaryFirst} 0%, ${palette.gradientSecondarySecond} 100%)`
  },
};
