...
const create132BitKeyWithSha256 = (
  generatedBytes: Buffer,
  random128bits: string,
) => {
...
export const generateNewKeys = (): GeneratedKey => {
  ...
  const random132bits = create132BitKeyWithSha256(
    generatedBytes,
    random128bits,
  );
  const generatedWords = generateWordsFromBytes(random132bits);
  const privateKey = pbkdf2Sync(
    generatedBytes,
    generatedBytes,
    512,
    32,
  ...
  const publicKey = curve.G.multiply(Bigi.fromBuffer(privateKey))
    .getEncoded(false)
    .toString('hex');
  return {
    publicKey,
